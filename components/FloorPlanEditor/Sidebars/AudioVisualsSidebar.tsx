...
export const AudioVisualsSidebar: React.FC = () => {
  ...
  const categories = ['All', ...getAllAVCategories()];

  const filteredItems = useMemo(() => {
    let items = AV_EQUIPMENT_LIBRARY;
  ...
  const crossResults = useGlobalSearch('Audio/Visual', searchQuery);
  ...
  return (
    <div className="flex h-full w-80 flex-col border-r border-gray-200 bg-white">
      ...
      <div className="border-b border-gray-200 bg-gray-50 px-4 py-3">
        <h3 className="text-lg font-semibold text-gray-900">Audio/Visual</h3>
        <p className="mt-1 text-xs text-gray-500">Drag equipment to canvas</p>
      </div>
